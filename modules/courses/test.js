...
describe('courses', () => {
    ...
    it('shoud not create a course - title not provided', async () => {
        const res = await request(server).post('/api/courses').send({ workLoad, places, teacher });
        expect(res.status).toBe(400);
    });

    it('shoud not create a course - workLoad not provided', async () => {
        const res = await request(server).post('/api/courses').send({ title, places, teacher });
        expect(res.status).toBe(400);
    });
    ...
    it('shoud not create a course - number of places not provided', async () => {
        const res = await request(server).post('/api/courses').send({ title, workLoad, teacher });
        expect(res.status).toBe(400);
    });
